...
---
title: "Response of aquatic weeds to herbicide treatments"
...
veg_data_north$Date<-mdy(veg_data_north$Date)
...
ggplot(data=vmass, aes(mass_kg)) + geom_histogram()
...
veg_data_north_stats<-veg_data_north %>% #name of the source tibble
  group_by(Site, Date) %>% #columns of tibble to group by
  summarize(
